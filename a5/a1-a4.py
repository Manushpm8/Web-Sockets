...
q = ""
while (q != "Q"):
    a5_fnx_inp.choice_command()
    z = input()
    if (z == "F"):
        # File System Input Statements
        a5_fnx_inp.file_input_command()
        n = input()
        x = n.strip().split()
        q = x[0]
        # Admin Mode
                ...
                if (len(x) > 2):           
                    if (x[2] == "-n"):
                        try:
                            ...
                            f = open(p, mode = "r")
                            a5_fnx_inp.E_command()
                            a5_fnx_inp.P_command()
                            a5_fnx_inp.Q_command()
                            m = input()
                            n = m.strip().split()
                            j = {}
                            for i in f:
    ...
    # This command adds and edits posts to the internet with new as well as existing user profiles.
        ...
        a5_fnx_inp.internet_input_command()
        srv = str(input("Server IP: "))
        port = 3021
        a5_fnx_inp.profile_input_command()
        nacl_enc = NaClProfile()
        nacl_enc.generate_keypair()
        js = ""
        js_msg = ds_client.communicator(srv, port, "", "", js, nacl_enc.public_key)
        pr = input()
        ...
        if (pr == "N"):
            ...
            js = ds_protocol.join_command(usr, pas, nacl_enc.public_key)
            js_msg = ds_client.communicator(srv, port, usr, pas, js, nacl_enc.public_key)
            ds_msg = json.loads(js_msg)
            tok = ds_msg["response"]["token"]
            a5_fnx_inp.post_input_command()
            ans = input()
            if(ans == "Yes"):
                a5_fnx_inp.keyword_input_command()
                ...
                post = nacl_enc.encrypt_entry(post, tok).decode(encoding = "UTF-8")
                po = Profile.Post(entry = post, timestamp = time.time())
                ts = str(po.get_time())
                js = ds_protocol.post_command(nacl_enc.public_key, post, ts)
                js_msg = ds_client.communicator(srv, port, usr, pas, js, nacl_enc.public_key)
            else:
                pass
        ...
        # Adds/Updates The Bio Of User Profiles On The DS Server
            ...
            bio = nacl_enc.encrypt_entry(bio, tok).decode(encoding = "UTF-8")
            b = [bio, time.time()]
            ts = str(b[1])
            js = ds_protocol.bio_command(nacl_enc.public_key, bio, ts)
            js_msg = ds_client.communicator(srv, port, usr, pas, js, nacl_enc.public_key)
