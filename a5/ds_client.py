...
def communicator(server:str, port:int, message:str):
    '''
...
def send(server:str, port:int, username:str, password:str, message:str, public_key:str, bio:str=None):
    '''
    The send function joins a ds server and sends a message, bio, or both
    :param server: The ip address for the ICS 32 DS server.
    :param port: The port where the ICS 32 DS server is accepting connections.
    ...
    try:
        js = "{\"join\": {\"username\": \"" + username + "\",\"password\": \"" + password + "\",\"token\": \"" + public_key + "\"}}"
        js_msg = communicator(server, port, username, password, js, public_key)
        ...
        final_msg = communicator(server, port, username, password, js, public_key)
